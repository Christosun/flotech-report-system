    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="bg-gradient-to-br from-red-50 to-rose-100 px-6 pt-6 pb-4 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-800">Delete Document?</h3>
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{files.length} saved document</p>
        </div>
      ...
      {hasFilter && (
        <p className="text-xs text-gray-400 mb-3 px-1">
          Display <span className="font-bold text-gray-700">{filtered.length}</span> from {files.length} document
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-500 font-medium">
            {hasFilter ? "No documents match the filter" : "There are no documents yet"}
          </p>
          {!hasFilter && (
            <button onClick={() => setShowUpload(true)}
                ...
                <div key={file.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col gap-1 flex-shrink-0">
                      {isPdf && (
                        <button onClick={() => handleView(file.id)} className="text-xs text-[#0B3D91] font-semibold hover:underline">View</button>
                      )}
                      ...
                      <button onClick={() => setDeleteTarget({ id: file.id, title: file.title })} className="text-xs text-red-500 font-semibold hover:underline">Delete</button>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                  placeholder="Document name" className={inputClass} />
              </div>
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                  rows={2} placeholder="Brief description of the contents of the document" className={inputClass + " resize-none"} />
              </div>

              <div>
                <label className={labelClass}>Tags <span className="text-gray-400 font-normal normal-case">(separate with commas)</span></label>
                <input value={form.tags} onChange={e => setForm({...form, tags: e.target.value})}
                  placeholder="flow, magnetic, wastewater, DN50, pressure" className={inputClass} />
                {/* Suggest existing tags */}
                {allTags.length > 0 && (
                  <div className="mt-2">
                    <p className="text-xs text-gray-400 mb-1.5">Existing tags (click to add):</p>
                    <div className="flex flex-wrap gap-1">
