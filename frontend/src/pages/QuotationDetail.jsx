    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 bg-[#0B3D91]">
        <span className="text-white font-bold text-sm flex items-center gap-2">
          <span className="opacity-70">📋</span> {quotationNumber} — Preview PDF
        </span>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {deleteDialog && (
        <DeleteDialog
          ...
          message={`"${q.quotation_number}" akan dihapus permanen.`}
      ...
      {!editMode && (
        <div className="space-y-5">
          ...
          {/* Items table — 6 cols, no Disc% column */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                      ...
                      <tr key={`item-${i}-main`} className={i%2===0 ? "bg-white" : "bg-gray-50/50"}>
                        ...
                        <td className="px-4 py-3">
                          ...
                          {parseFloat(item.discount) > 0 && (
                            <span className="inline-block mt-1 px-1.5 py-0.5 bg-orange-100 text-orange-700 text-[10px] font-bold rounded-full">
                              Disc {item.discount}%
                            </span>
                          )}
                        </td>
                ...
                <tfoot className="border-t-2 border-gray-200">
                  {hasDiscount && (
                    <tr key="tfoot-gross">
                      <td colSpan="5" className="px-4 py-2 text-right text-xs text-gray-400 font-semibold">Subtotal (sebelum diskon)</td>
                  ...
                  {hasDiscount && (
                    <tr key="tfoot-disc" className="bg-orange-50/60">
                      <td colSpan="5" className="px-4 py-2 text-right text-xs font-bold text-orange-600">Total Diskon</td>
      ...
      {editMode && form && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="p-6 space-y-7">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {/* No. Quotation — READ ONLY, tidak bisa diubah */}
                <Field
                ...
                {form.items.map((item, i) => (
                  <div key={`edit-item-${i}`} className="bg-gray-50/80 rounded-xl p-4 border border-gray-100">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {form.items.length > 1 && (
                        <button onClick={() => rmItem(i)} className="text-xs text-red-400 hover:text-red-600 font-semibold">Hapus Item</button>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100 gap-2">
                      <button onClick={() => addSubItem(i)}
                        className="text-[10px] font-bold text-blue-500 hover:text-blue-700 hover:underline shrink-0">
              ...
              {(() => {
                ...
                return (
                  <div className="mt-2 p-4 bg-[#0B3D91]/5 rounded-xl border border-[#0B3D91]/10 text-right space-y-1.5">
                    {hasD && <p><span className="text-xs text-orange-500">Total Diskon: </span><span className="text-sm font-bold text-orange-600">({fmtRp(disc,form.currency)})</span></p>}
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between gap-3 bg-gray-50/50 rounded-b-2xl">
            <p className="text-xs text-gray-400">Perubahan item/harga/terms akan auto-increment revision</p>
            <div className="flex gap-3">
              <button onClick={() => setEditMode(false)}
                className="px-5 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-semibold hover:bg-white">
                Batal
              </button>
              <button onClick={handleSave} disabled={saving}
