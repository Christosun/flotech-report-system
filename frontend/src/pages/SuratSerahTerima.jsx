...
const SURAT_TYPE = {
  serah: { label: "Handover", icon: "📤", bg: "bg-blue-100", text: "text-blue-700" },
...
export default function SuratSerahTerima() {
  ...
  const stats = [
    ...
    { label: "Handover", val: items.filter(i => i.surat_type === "serah").length, icon: "📤", color: "text-blue-600" },
    { label: "Receive Goods", val: items.filter(i => i.surat_type === "terima").length, icon: "📥", color: "text-emerald-600" },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="relative lg:col-span-2">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search for letter number, name, company..." className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0B3D91]" />
          </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-4 text-right flex-shrink-0">
                    <div>
                      ...
                      {s.items_count > 0 && <p className="text-xs text-gray-400">{s.items_count} items</p>}
                    </div>
