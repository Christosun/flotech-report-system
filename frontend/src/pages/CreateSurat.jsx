...
export default function CreateSurat() {
  ...
  const handleSubmit = async () => {
    if (!form.pihak_pertama_nama || !form.pihak_kedua_nama) { toast.error("The names of the first and second parties must be filled in."); return; }
    if (!form.barang_items.some(i => i.nama_barang)) { toast.error("Minimum content of 1 item"); return; }
    setSaving(true);
    try {
      const res = await API.post("/surat/create", form);
      toast.success("The letter was created successfully! 📜");
      navigate(`/surat/${res.data.id}`);
    } catch (err) { toast.error(err.response?.data?.error || "Failed to write a letter"); }
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Create Material Handover</h1>
        <p className="text-gray-400 text-sm mt-1">Professional and modern minutes</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="flex gap-3 mb-4">
            {Object.entries({ serah: { label: "Handover of Goods/Services", icon: "📤", desc: "Handing over goods/services to another party" }, terima: { label: "Receive Goods", icon: "📥", desc: "Receiving goods from other parties" } }).map(([k, v]) => (
              <button key={k} type="button" onClick={() => set("surat_type", k)}
                ...
                {form.surat_type === k && <p className="text-xs text-[#0B3D91] font-semibold mt-2">✓ Selected</p>}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2"><label className={labelClass}>Subject</label><input value={form.perihal} onChange={e => set("perihal", e.target.value)} placeholder="Example: Delivery of Flow Meter for XYZ project" className={inputClass} /></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-3">
              <div><label className={labelClass}>Name *</label><input value={form.pihak_pertama_nama} onChange={e => set("pihak_pertama_nama", e.target.value)} placeholder="Full Name" className={inputClass} /></div>
              <div><label className={labelClass}>Position</label><input value={form.pihak_pertama_jabatan} onChange={e => set("pihak_pertama_jabatan", e.target.value)} placeholder="Sales Manager" className={inputClass} /></div>
              <div><label className={labelClass}>Company</label><input value={form.pihak_pertama_perusahaan} onChange={e => set("pihak_pertama_perusahaan", e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Address</label><textarea value={form.pihak_pertama_alamat} onChange={e => set("pihak_pertama_alamat", e.target.value)} rows={2} className={inputClass + " resize-none"} /></div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-3">
              <div><label className={labelClass}>Name *</label><input value={form.pihak_kedua_nama} onChange={e => set("pihak_kedua_nama", e.target.value)} placeholder="Full Name" className={inputClass} /></div>
              <div><label className={labelClass}>Position</label><input value={form.pihak_kedua_jabatan} onChange={e => set("pihak_kedua_jabatan", e.target.value)} placeholder="Procurement Manager" className={inputClass} /></div>
              <div><label className={labelClass}>Company</label><input value={form.pihak_kedua_perusahaan} onChange={e => set("pihak_kedua_perusahaan", e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Address</label><textarea value={form.pihak_kedua_alamat} onChange={e => set("pihak_kedua_alamat", e.target.value)} rows={2} className={inputClass + " resize-none"} /></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={addItem} className="px-3 py-1.5 bg-[#0B3D91] text-white rounded-lg text-xs font-semibold hover:bg-[#1E5CC6]">+ Add Item</button>
          </div>
          <div className="hidden sm:grid grid-cols-12 gap-2 mb-1 px-1">
            {["No", "Items Name", "Qty", "Unit", "Description", ""].map(h => (
              <p key={h} className={`text-[10px] font-bold text-gray-400 uppercase tracking-wide ${h === "Items Name" ? "col-span-4" : h === "Description" ? "col-span-3" : "col-span-1"}`}>{h}</p>
            ))}
          </div>
          <div className="space-y-2">
            {form.barang_items.map((item, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 bg-gray-50 rounded-xl p-2.5 items-center">
                ...
                <div className="col-span-4 sm:col-span-4"><input value={item.nama_barang} onChange={e => updateItem(i, "nama_barang", e.target.value)} placeholder="Items Name" className={inputClass} /></div>
                ...
                <div className="col-span-2"><input value={item.keterangan} onChange={e => updateItem(i, "keterangan", e.target.value)} placeholder="Notes" className={inputClass} /></div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <textarea value={form.catatan} onChange={e => set("catatan", e.target.value)} rows={3} placeholder="Item condition, special notes, etc..." className={inputClass + " resize-none"} />
        </div>
      ...
      <div className="flex gap-3 mt-6">
        <button onClick={() => navigate("/surat")} className="px-5 py-3 border border-gray-200 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-50">Cancel</button>
        <button onClick={handleSubmit} disabled={saving}
          ...
          {saving ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Saving…</> : "📜 Create a Handover Letter"}
        </button>
      </div>
    </div>
  );
