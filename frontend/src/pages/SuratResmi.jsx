...
export default function SuratResmi() {
  ...
  const handleCreate = async () => {
    if (!form.perihal) { toast.error("Subject must be filled in"); return; }
    setSaving(true);
    try {
    ...
    } catch (e) { toast.error(e.response?.data?.error || "Failed to write a letter"); }
          ...
          <div className="space-y-4">
            {editMode ? (
              ...
              <>
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <h3 className="text-xs font-bold text-[#0B3D91] uppercase tracking-wider mb-4 flex items-center gap-2">
                    <span className="w-1.5 h-4 bg-[#0B3D91] rounded-full" /> Letter Information
                  </h3>
                  <div className="space-y-2 text-sm">
                    {[
                      ["Number", selected.nomor],
                      ["Date", formatDateID(selected.surat_date)],
                      ["Subject", selected.perihal],
                      ["Attachment", selected.lampiran],
                    ].filter(([, v]) => v).map(([l, v]) => (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <h3 className="text-xs font-bold text-[#0B3D91] uppercase tracking-wider mb-4 flex items-center gap-2">
                    <span className="w-1.5 h-4 bg-[#0B3D91] rounded-full" /> Addressed to
                  </h3>
                  <div className="space-y-2 text-sm">
                    {[
                      ["Name", selected.kepada_nama],
                      ["Position", selected.kepada_jabatan],
                      ["Company", selected.kepada_perusahaan],
                      ["Address", selected.kepada_alamat],
                    ].filter(([, v]) => v).map(([l, v]) => (
                      <div key={l} className="flex gap-3">
                        <span className="text-gray-400 w-20 flex-shrink-0">{l}</span>
