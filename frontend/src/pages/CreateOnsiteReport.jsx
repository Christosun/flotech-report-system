...
import {
  ArrowLeft, FileText, Building2, User, Wrench, ClipboardList, PenLine,
  Plus, X, Bold, Italic, Underline, List, ListOrdered, AlignLeft,
  AlignCenter, AlignRight, AlignJustify, Image, Rocket, Loader2,
  Calendar, Info, Trash2, CheckCircle2,
} from "lucide-react";
    ...
    <div>
      ...
      {value ? (
        <div className="border border-gray-200 rounded-xl bg-gray-50 p-3 relative">
          ...
          <button onClick={() => { onChange(""); setHasDrawn(false); }}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowPad(false)} className="text-gray-400 hover:text-gray-600"><X size={18} /></button>
            </div>
...
function RichTextEditor({ value, onChange }) {
  ...
  const ToolBtn = ({ cmd, val, title, children }) => (
    <button type="button" title={title} onMouseDown={e => { e.preventDefault(); exec(cmd, val); }}
      className="p-1.5 hover:bg-gray-100 rounded transition-colors text-gray-600">{children}</button>
  );
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 bg-gray-50 border-b border-gray-200">
        ...
        <div className="w-px h-5 bg-gray-200 mx-1" />
        <label title="Font Color" className="flex items-center gap-1 px-1.5 py-1.5 hover:bg-gray-100 rounded cursor-pointer">
        ...
        <button type="button" title="Insert Image" onClick={() => fileInputRef.current?.click()}
          className="p-1.5 hover:bg-gray-100 rounded transition-colors text-gray-600"><Image size={14} /></button>
      ...
      <div ref={editorRef} contentEditable suppressContentEditableWarning onInput={handleInput} onClick={handleEditorClick}
        className="rich-editor min-h-[200px] p-4 text-sm text-gray-700 focus:outline-none" style={{ lineHeight: "1.6" }}
        data-placeholder="Write a job description here..." />
      <style>{`
...
function EquipmentItem({ item, index, onChange, onRemove, canRemove }) {
  ...
  return (
    <div className="border border-gray-100 rounded-xl p-4 bg-gray-50 relative">
      {canRemove && (
        <button onClick={() => onRemove(index)} className="absolute top-3 right-3 text-red-400 hover:text-red-600 transition-colors">
          <X size={16} />
        </button>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <input value={item.description} onChange={e => onChange(index, "description", e.target.value)}
            placeholder="Name/type, e.g. Flow Transmitter" className={inputCls} />
        </div>
        <div>
        ...
        <div>
          ...
          <input value={item.serial_number} onChange={e => onChange(index, "serial_number", e.target.value)}
            placeholder="e.g. 20240001" className={inputCls} />
        </div>
      </div>
    </div>
  );
...
export default function CreateOnsiteReport() {
  ...
  const generateReportNumber = async () => {
    ...
    try {
      ...
      const sameDay = reports.map(r => r.report_number || "").filter(n => n.startsWith(prefix))
        .map(n => parseInt(n.replace(prefix, ""), 10)).filter(n => !isNaN(n));
      const nextNum = sameDay.length > 0 ? Math.max(...sameDay) + 1 : 1;
      setForm(f => ({ ...f, report_number: `${prefix}${String(nextNum).padStart(3, "0")}` }));
    } catch {
      setForm(f => ({ ...f, report_number: `OSR-${dateStr}-001` }));
    }
  };
    ...
    <div className="w-full">
      ...
      <div className="space-y-4">
        ...
        <PDFSettingsBar
          ...
          onLangChange={v => set("pdf_language", v)}
          inclSig={form.include_customer_signature}
          onSigChange={v => set("include_customer_signature", v)}
        />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="date" value={form.visit_date_from}
                onChange={e => set("visit_date_from", e.target.value)} className={inputClass} />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.client_company} onChange={e => set("client_company", e.target.value)}
                placeholder="Name of company/agency" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Contact Person</label>
              <input value={form.contact_person} onChange={e => set("contact_person", e.target.value)}
                placeholder="PIC name/contact" className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.site_location} onChange={e => set("site_location", e.target.value)}
                placeholder="Job site location" className={inputClass} />
            </div>
            <div className="sm:col-span-2">
              <label className={labelClass}>Address</label>
              <input value={form.client_address} onChange={e => set("client_address", e.target.value)}
                placeholder="Complete address" className={inputClass} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs font-bold text-[#0B3D91] uppercase tracking-wider flex items-center gap-2">
              <span className="w-5 h-5 bg-[#0B3D91] text-white rounded-full flex items-center justify-center text-xs font-black flex-shrink-0">4</span>
          ...
          <div className="space-y-3">
            {form.equipment_items.map((item, idx) => (
              <EquipmentItem key={idx} item={item} index={idx}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <RichTextEditor value={form.job_description} onChange={v => set("job_description", v)} />
          <p className="text-xs text-gray-400 mt-1.5">Use the toolbar to format text, add lists, font colors, or images.</p>
        </div>

        {/* 6 — Customer Signature (only if inclSig) */}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="mt-3 bg-blue-50 rounded-xl px-4 py-3 flex items-start gap-2">
              ...
              <p className="text-xs text-blue-700">
                <strong>Engineer signature</strong> is taken automatically from the data registered in the Engineers menu.
              </p>
            </div>
          </div>
        )}

        {/* 6 — Reported By preview (only if !inclSig) */}
        {!form.include_customer_signature && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button onClick={handleSubmit} disabled={saving}
          ...
          {saving
            ...
            : <><Rocket size={16} /> Create an Onsite Report</>}
        </button>
      </div>
    </div>
  );
