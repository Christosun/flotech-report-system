    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">Unit Category</label>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">Status Filter</label>
...
export default function Stock() {
  ...
  const handleBatchSubmit = async () => {
    ...
    if (validVariants.length === 0) { toast.error("Minimum content 1 variant (model or serial number)"); return; }
    setSaving(true);
    try {
  ...
  return (
    <div>
      ...
      {deleteTarget && (
        <DeleteDialog
          title="Delete Unit?"
          description={`"${deleteTarget.name} — ${deleteTarget.model || deleteTarget.serial_number || ""}" will be permanently deleted.`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Inventory Equipment/Instrument Flotech</p>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["Equipment/Instrument Name", "Brand/Model", "S/N", "Category", "Status", "Condition", "Location", "Action"].map(h => (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-2.5 text-sm">
              {[
                ...
                ["Purchase Price", viewItem.purchase_price && new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(viewItem.purchase_price)],
              ...
              {viewItem.description && <div><p className="text-gray-400 mb-1">Description</p><p className="text-gray-800">{viewItem.description}</p></div>}
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div><label className={labelClass}>Location</label><input value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="Warehouse/Office/Site" className={inputClass} /></div>
                  <div><label className={labelClass}>Borrowed To</label><input value={form.loan_to} onChange={e => setForm({ ...form, loan_to: e.target.value })} placeholder="Company Name" className={inputClass} /></div>
                  <div><label className={labelClass}>Borrow Date</label><input type="date" value={form.loan_date} onChange={e => setForm({ ...form, loan_date: e.target.value })} className={inputClass} /></div>
                  <div><label className={labelClass}>Return Date</label><input type="date" value={form.return_date} onChange={e => setForm({ ...form, return_date: e.target.value })} className={inputClass} /></div>
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2"><label className={labelClass}>Description</label><textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className={inputClass + " resize-none"} /></div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-4">
            ...
            <div className="p-6">
              ...
              <div className="hidden sm:grid grid-cols-6 gap-2 mb-1 px-1">
                {["Model/Size", "Serial Number", "Condition", "Status", "Location", "Remarks"].map(h => (
