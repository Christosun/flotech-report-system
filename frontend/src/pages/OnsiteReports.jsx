  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-gray-100 bg-white rounded-b-2xl">
      ...
      <span className="text-xs text-gray-400 font-medium">
        {total === 0 ? "0 data" : `${from}–${to} from ${total}`}
      </span>
...
function BulkActionBar({ selectedIds, allIds, onSelectAll, onClearAll, onBulkDeleted }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`${selectedIds.length} onsite report deleted`);
  ...
  return (
    <>
      {showDeleteConfirm && (
        <DeleteDialog
          title={`Delete ${selectedIds.length} Onsite Report?`}
          description="All selected onsite reports will be permanently deleted. This action cannot be undone."
    ...
    <div className="w-full">
      ...
      {showFilter && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
          ...
          {activeFiltersCount > 0 && (
            <p className="text-xs text-gray-400 mt-3">{filtered.length} from {reports.length} report is showing</p>
          )}
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        {[["", "All"], ...Object.entries(STATUS_CONFIG).map(([k,v]) => [k, v.label])].map(([st, lbl]) => (
          <button key={st}
