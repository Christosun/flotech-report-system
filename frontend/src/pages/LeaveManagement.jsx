...
const LEAVE_TYPES = [
  ...
  { value: "latepermission", label: "Late Permission",icon: "⏰",  color: "#24bdb0" },
];
  ...
  return (
    <div className={`bg-white rounded-2xl border ${highlight ? "border-emerald-200 shadow-emerald-50" : "border-gray-100"} shadow-sm p-4 lg:p-5`}>
      <div className="flex items-start justify-between mb-3">
        ...
        {highlight && <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full tracking-wide">LEFT</span>}
      </div>
...
function DetailModal({ req: r, onClose }) {
  ...
  const rows = [
    ...
    ["Leave Type",       `${lt?.icon || ""} ${lt?.label || r.leave_type}`],
    ["Reason",           r.reason],
  ...
  return (
    <Modal onClose={onClose} title="Leave Application Details" maxW="max-w-sm">
...
function ApprovalModal({ req: r, onClose, onDecide }) {
  ...
  return (
    <Modal onClose={onClose} title="⚖️ Application Review">
...
function JointScheduleModal({ onClose, onSaved, year }) {
  ...
  return (
    <Modal onClose={onClose} title={`➕ Add Joint Leave ${year}`} maxW="max-w-sm">
...
function EntitlementModal({ users, year, onClose, onSaved }) {
  ...
  return (
    <Modal onClose={onClose} title={`⚙️ Employee Leave Entitlement ${year}`} maxW="max-w-md">
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 className="text-sm font-black text-gray-800 mb-4 flex items-center gap-2">📊 Annual Leave Balance</h3>
        <div className="mb-5">
      ...
      {/* Usage by type */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <h3 className="text-sm font-black text-gray-800 mb-4">📋 Usage per Type</h3>
        {byType.length === 0 ? (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 lg:col-span-2">
        ...
        {myReqs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-gray-400">
            <span className="text-4xl mb-2">🏖️</span><p className="text-sm">There are no leave requests yet</p>
          </div>
        ) : (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} className="text-center py-12 text-gray-400">
                  <p className="text-3xl mb-2">📋</p><p className="text-sm">No data</p>
                </td></tr>
              ) : filtered.map((r, i) => {
                const lt = LEAVE_TYPES.find(t => t.value === r.leave_type);
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {r.status === "pending" && (isOwn || isAdmin) && (
                          <button onClick={() => onDelete(r.id)} className="text-xs text-red-400 font-bold hover:text-red-600">Cancel</button>
                        )}
                      </div>
                    </td>
                  </tr>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-black text-gray-800">Joint Leave Schedule {year}</h3>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-purple-50 border-b border-purple-100">
              ...
              <th className="text-center px-5 py-3 text-xs font-bold text-purple-700">Days</th>
...
function ApprovalsTab({ pending, onReview }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500 font-semibold px-1">{pending.length} application pending decision</p>
      {pending.map(r => {
        const lt = LEAVE_TYPES.find(t => t.value === r.leave_type);
        return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Leave Entitlement", val: totalEntitlement, icon: "📋", color: "text-[#0B3D91] bg-blue-50"   },
          { label: "Total Taken",  val: totalTaken,        icon: "✈️", color: "text-amber-600 bg-amber-50"  },
          { label: "Total Balance",    val: totalBalance,      icon: "💚", color: "text-emerald-600 bg-emerald-50" },
          { label: "Pending",       val: totalPending,      icon: "⏳", color: "text-purple-600 bg-purple-50" },
        ].map(s => (
        ...
        {filtered.map(u => (
          <div key={u.user_id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: "Leave Entitlement",   val: u.entitlement,  color: "text-[#0B3D91]", bg: "bg-blue-50"    },
                { label: "Joint Leave",    val: u.joint_leave,  color: "text-purple-600",bg: "bg-purple-50"  },
                { label: "Leave taken",    val: u.annual_taken, color: "text-amber-600", bg: "bg-amber-50"   },
...
export default function LeaveManagement() {
  ...
  const TABS = [
    ...
    { id: "joint",     label: "Joint Leave",     icon: "🤝" },
    ...(isAdmin ? [
      { id: "rekap",     label: "Recap",       icon: "👥" },
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 lg:p-6">
        ...
        {loading ? <Spinner className="h-24" /> : (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label="Annual Leave Entitlement"   value={summary.entitlement  || 12} icon="📋" color="#0B3D91" />
            <StatCard label="Joint Leave"        value={summary.joint_leave  || 0}  icon="🤝" color="#7c3aed" />
            <StatCard label="Leave Taken"        value={summary.annual_taken || 0}  icon="✈️" color="#d97706" />
            <StatCard label="Remaining Balance"       value={summary.balance      ?? 0}  icon="💚" color="#059669" highlight />
          </div>
        )}
      </div>
