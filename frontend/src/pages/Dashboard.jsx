...
function StatCard({ icon, label, value, sub, color, iconColor, onClick }) {
  return (
    <button
      onClick={onClick}
    ...
    <div>
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          {greeting}, {userName}{" "}
          <i className="fas fa-hand-wave text-yellow-400 text-xl"></i>
        </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <StatCard
          ...
          label="Onsite Reports"
          value={stats.onsite}
          sub={`${stats.onsiteDraft} draft`}
          color="bg-cyan-50"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="bg-gradient-to-r from-[#0B3D91] to-[#1E5CC6] rounded-2xl p-5 text-white">
          <p className="text-xs font-bold text-blue-200 uppercase tracking-wider mb-1">
            <i className="fas fa-coins mr-1.5"></i>Won Revenue
          </p>
        ...
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-5 text-white">
          <p className="text-xs font-bold text-orange-100 uppercase tracking-wider mb-1">
            <i className="fas fa-chart-line mr-1.5"></i>Pipeline Value
          </p>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {[
            { icon: "fas fa-pen-to-square", label: "Official Report",   link: "/reports/create", color: "bg-blue-50 hover:bg-blue-100 text-blue-700" },
            { icon: "fas fa-screwdriver-wrench", label: "Onsite Report", link: "/onsite/create",  color: "bg-cyan-50 hover:bg-cyan-100 text-cyan-700" },
            { icon: "fas fa-file-invoice",  label: "Quotation",          link: "/quotations",     color: "bg-indigo-50 hover:bg-indigo-100 text-indigo-700" },
            { icon: "fas fa-file-signature",label: "Material Handover",  link: "/surat/create",   color: "bg-purple-50 hover:bg-purple-100 text-purple-700" },
            { icon: "fas fa-cube",          label: "Add Unit",            link: "/stock",          color: "bg-orange-50 hover:bg-orange-100 text-orange-700" },
            { icon: "fas fa-upload",        label: "Upload Catalog",      link: "/catalog",        color: "bg-teal-50 hover:bg-teal-100 text-teal-700" },
          ].map(a => (
            <button
              key={a.label}
...
function RecentCard({ icon, title, link, empty, emptyMsg = "No data yet", children }) {
  const navigate = useNavigate();
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-4 py-3.5 border-b border-gray-50 flex items-center justify-between">
        <h2 className="font-bold text-gray-800 text-sm flex items-center gap-2">
          <i className={`${icon} text-[#0B3D91] text-xs`}></i>
          {title}
        </h2>
