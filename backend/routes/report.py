...
# ── DITAMBAHKAN: Bilingual signature labels (sinkron dengan onsite_report.py) ──
SIGNATURE_LABELS = {
    "en": {
        "engineer":       "ENGINEER",
...
# ─────────────────────────────────────────────────────────────────────────────
# PDF BUILDER — bilingual, include_client_signature toggle
# Disinkronkan sepenuhnya dengan onsite_report.py:
#   - include_client_signature = True  → dual-column: Engineer (kiri) | Client (kanan)
#   - include_client_signature = False → single full-width "Reported By" box elegan
# ─────────────────────────────────────────────────────────────────────────────
def build_report_pdf(report_id):
    ...
    # ── Baca settings dari data_json ──────────────────────────────
    lang = data.get("_lang", "en")
    if lang not in ("en", "id"):
        lang = "en"

    # PERBAIKAN UTAMA: konsisten dengan frontend — key "_include_client_signature"
    # Default True (include signature) jika belum di-set
    include_client_sig = data.get("_include_client_signature")
    if include_client_sig is None:
        include_client_sig = True
    else:
        include_client_sig = bool(include_client_sig)
    ...
    # ── SIGNATURE SECTION ─────────────────────────────────────────
    # Helper: render signature image dari base64
    def _sig_image(b64_data):
        """Konversi base64 signature ke ReportLab Image. Return Spacer jika gagal."""
        if not b64_data:
        ...
        try:
            ...
            sig_buf.seek(0)
            img = Image(sig_buf, width=4*cm, height=1.5*cm)
            img.hAlign = 'CENTER'
            return img
        except:
            return Spacer(1, 1.8*cm)
    ...
    # Digital notice — sama untuk kedua mode
    ...
    if include_client_sig:
        # ── MODE 1: DUAL SIGNATURE — Engineer (kiri) | Client (kanan) ──
        # Identik dengan onsite_report.py inc_sig=True
        sig_rows = [
            [Paragraph(lbl["engineer"], sig_label_style),
             Paragraph(lbl["client"],   sig_label_style)],
            [_sig_image(engineer.signature_data if engineer else None),
             Spacer(1, 1.8*cm)],  # Kolom client: kosong untuk tanda tangan manual
            [HRFlowable(width=HALF_W - 1.5*cm, thickness=0.5, color=border_gray),
             HRFlowable(width=HALF_W - 1.5*cm, thickness=0.5, color=border_gray)],
            [Paragraph(engineer.name if engineer else "Engineer", sig_sub_style),
             Paragraph(lbl["name_stamp"], sig_sub_style)],
        ]
        # Baris extra: posisi engineer & tanggal client
        if engineer:
            pos_str = f"{engineer.position or ''}"
            if engineer.employee_id:
                pos_str += f"  |  {engineer.employee_id}"
            sig_rows.append([
                Paragraph(pos_str, sig_sub_style),
        ...
        # ── MODE 2: REPORTED BY ONLY — full-width elegan ─────────────
        # Identik dengan onsite_report.py inc_sig=False
        reporter_name = engineer.name     if engineer else "—"
        reporter_pos  = engineer.position if engineer else ""
        reporter_id   = (f"  |  {engineer.employee_id}" if engineer and engineer.employee_id else "") if engineer else ""

        rb_title = Paragraph(
            lbl["reported_by"],
            ps('RBT', fontSize=8, fontName='Helvetica-Bold', textColor=primary_color, alignment=0)
        )
        rb_name = Paragraph(
            f"<b>{reporter_name}</b>",
            ps('RBN', fontSize=13, fontName='Helvetica-Bold', textColor=dark_color, alignment=0, leading=16)
        )
        rb_pos = Paragraph(
            f"{reporter_pos}{reporter_id}",
            ps('RBP', fontSize=9, textColor=gray_color, alignment=0)
        ) if reporter_pos or reporter_id else Spacer(1, 0.1*cm)
        ...
        rb_text_col = Table([
            ...
            [rb_name],
            [rb_pos],
            [HRFlowable(width=text_col_w - 1*cm, thickness=0.5, color=border_gray)],
        ...
        sig_t = Table(
            [[rb_text_col, _sig_image(engineer.signature_data if engineer else None)]],
            colWidths=[text_col_w, sig_col_w]
        )
        sig_t.setStyle(TableStyle([
            ('ALIGN',      (0, 0), (-1, -1), 'LEFT'),
    ...
    # ── Gabungkan signature block (KeepTogether agar tidak terpotong halaman) ──
