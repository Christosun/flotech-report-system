...
def export_excel():
    ...
    for i, q in enumerate(qs):
        ...
        vals = [
            i + 1, q.quotation_number, q.customer_company or "",
            q.customer_name or "", q.project_name or "", q.sales_person or "",
            q.currency or "IDR", q.total_amount or 0,
...
def export_pdf_list():
    ...
    if logo_path:
        try:
            ...
            lw = 4.5 * cm; lh = lw * pil.size[1] / pil.size[0]
...
def build_quotation_pdf(q):
    ...
    def proj_block():
        ...
        if q.valid_until:
            rows.append([Paragraph(f"Valid Until : {q.valid_until.strftime('%d %B %Y')}",
    ...
    row_idx    = 1   # header = row 0; data rows start at 1
 
    for i, item in enumerate(items_list):
        sub_items = item.get("sub_items") or []
        has_subs  = len(sub_items) > 0
 
        # Build description flowable — multiline-safe via <br/>
        desc_flow = []
        ...
        if has_subs:
            # Main item row — description only, price cols empty
            sub_net = 0.0
            sub_disc = 0.0
            for s in sub_items:
            ...
            for si, s in enumerate(sub_items):
                ...
                disc_p_s = (
                    Paragraph(f"<b>{sd:.0f}%</b>",
                        S(f"sdc{i}_{si}", fontName="Helvetica-Bold", fontSize=8,
                          textColor=C_DISC_FC, leading=11, alignment=TA_CENTER))
                    if sd > 0 else Paragraph("—", TDC)
                )
    ...
    if has_disc:
        r_idx = len(tot_rows)
        tot_rows.append([Paragraph("Discount :", DL), Paragraph(f"({fmt(total_disc)})", DV)])
        tot_styles += [
    ...
    # ── SECTION 6: Terms + Regards (side by side) ────────────────────────────
